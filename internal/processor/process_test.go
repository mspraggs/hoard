...
func (s *ProcessorTestSuite) TestProcess() {
	...
	s.Run("creates and uploads file", func() {
		s.Run("where file previously uploaded", func() {
			...
			processor := processor.New(fs, keyGen, s.mockUploader, s.mockRegistry)
	...
	s.Run("file with matching checksum not uploaded", func() {
		prevFile := &processor.File{
			Key:       key,
			LocalPath: path,
			Checksum:  checksum,
	...
	s.Run("handles error", func() {
		...
		s.Run("when file not found", func() {
			path := "file/not/found"
			processor := processor.New(fs, nil, nil, nil)
		...
		s.Run("from fetch latest", func() {
			...
			processor := processor.New(fs, keyGen, nil, s.mockRegistry)
